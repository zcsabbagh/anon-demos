...
import puppeteerPlugin from "puppeteer-extra-plugin-stealth";
import { z } from "zod";
export const managedBrowserEnvironmentInputSchema = z.object({
    apiKey: z.string(),
    env: z.string(),
...
export async function getBrowserRuntimeEnvironment(input) {
...
export async function initializeAnonLocalBrowser(input) {
    chromium.use(puppeteerPlugin());
    let browser;
    if (input.proxy && "server" in input.proxy) {
        const proxy = input.proxy;
        browser = await chromium.launch({ ...input, proxy });
    }
    else {
...
export async function initializeAnonManagedBrowser(input) {
    const runtimeEnvironment = await getBrowserRuntimeEnvironment(input);
    const browser = await chromium.connectOverCDP(runtimeEnvironment.environment.connectUrl);
    return proxyBrowser(browser, {
        type: "managed",
        input: { ...input, runtimeEnvironment },
    });
}
export async function initializeAnonRemoteBrowser(input) {
    const browser = await chromium.connectOverCDP(input);
    return proxyBrowser(browser, { type: "remote", input });
}
export async function initializeAnonBrowser(options) {
...
export async function setupAnonBrowserContext(browser, proxy, // HACK: We have two types of proxies, LocalProxyConfig and Proxy, for backwards compatibility
contextOptions) {
...
export async function setupAnonBrowserWithContext(browserLaunchOptions, proxy, // HACK: We have two types of proxies, LocalProxyConfig and Proxy, for backwards compatibility
contextOptions) {
...
export async function addAnonSession(page, session) {
    try {
...
export function makePlaywrightCookie(cookie) {
    return {
        ...cookie,
...
export function extractProxyConfig(proxy) {
    if (!proxy)
        return;
    const regex = /^([^:]+):\/\/([^:]+):([^@]+)@([^:]+):(\d+)$/;
...
export class FailedToAddSessionError extends Error {
    sessionId;
    cause;
...
export function proxyBrowser(browser, browserConfig) {
        ...
        get(target, prop, receiver) {
            if (prop === "newContext") {
                ...
                return async function (...args) {
                    ...
                    // Proxy the context
                    const proxiedContext = proxyBrowserContext(context, browserConfig);
