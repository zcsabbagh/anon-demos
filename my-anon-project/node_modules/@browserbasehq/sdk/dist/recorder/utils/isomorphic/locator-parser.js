...
const locator_generators_1 = require("./locator-generators");
...
function parseLocator(locator, testIdAttributeName) {
    ...
    template = template
        ...
        .replace(/regex=/g, '=')
        .replace(/,,/g, ',');
    const preferredQuote = params
        .map((p) => p.quote)
        .filter((quote) => '\'"`'.includes(quote))[0];
    return {
...
function transform(template, params, testIdAttributeName) {
    ...
    return parts
        ...
        t = t
            ...
            const param = params[+ordinal - 1];
            if (t.startsWith('internal:attr') ||
                t.startsWith('internal:testid') ||
                t.startsWith('internal:role'))
                return ((0, string_utils_1.escapeForAttributeSelector)(new RegExp(param.text), false) +
                    (suffix || ''));
            return (0, string_utils_1.escapeForTextSelector)(new RegExp(param.text, suffix), false);
        })
            ...
            if (t.startsWith('internal:testid'))
                return (0, string_utils_1.escapeForAttributeSelector)(param.text, true);
            if (t.startsWith('internal:attr') || t.startsWith('internal:role'))
                return (0, string_utils_1.escapeForAttributeSelector)(param.text, suffix === 's');
            return (0, string_utils_1.escapeForTextSelector)(param.text, suffix === 's');
        });
        return t;
    })
        .join(' >> ');
...
function locatorOrSelectorAsSelector(language, locator, testIdAttributeName) {
    try {
        (0, selector_parser_1.parseSelector)(locator);
        return locator;
    }
    catch (e) { }
    try {
        const { selector, preferredQuote } = parseLocator(locator, testIdAttributeName);
        const locators = (0, locator_generators_1.asLocators)(language, selector, undefined, undefined, preferredQuote);
