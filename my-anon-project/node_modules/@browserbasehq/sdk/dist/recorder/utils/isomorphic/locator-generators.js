...
exports.JsonlLocatorFactory = exports.CSharpLocatorFactory = exports.JavaLocatorFactory = exports.PythonLocatorFactory = exports.JavaScriptLocatorFactory = void 0;
...
function asLocators(lang, selector, isFrameLocator = false, maxOutputSize = 20, preferredQuote) {
    try {
        return innerAsLocators(new generators[lang](preferredQuote), (0, selector_parser_1.parseSelector)(selector), isFrameLocator, maxOutputSize);
    }
    catch (e) {
...
function innerAsLocators(factory, parsed, isFrameLocator = false, maxOutputSize = 20) {
    ...
    for (let index = 0; index < parts.length; index++) {
        ...
        if (part.name === 'internal:role') {
            const attrSelector = (0, selector_parser_1.parseAttributeSelector)(part.body, true);
            const options = { attrs: [] };
        ...
        if (nextPart &&
            ...
            nextPart.body === 'enter-frame') {
            locatorType = 'frame';
            nextBase = 'frame-locator';
            index++;
        }
        const selectorPart = (0, selector_parser_1.stringifySelector)({ parts: [part] });
        const locatorPart = factory.generateLocator(base, locatorType, selectorPart);
        if (locatorType === 'default' &&
            nextPart &&
...
class JavaScriptLocatorFactory {
    ...
    regexToSourceString(re) {
        return (0, string_utils_1.normalizeEscapedRegexQuotes)(String(re));
    }
    ...
    quote(text) {
        return (0, string_utils_1.escapeWithQuotes)(text, this.preferredQuote ?? "'");
    }
}
...
class PythonLocatorFactory {
        ...
        switch (kind) {
            ...
            case 'role':
                ...
                for (const { name, value } of options.attrs) {
                    ...
                    attrs.push(`${(0, string_utils_1.toSnakeCase)(name)}=${valueString}`);
                }
                const attrString = attrs.length ? `, ${attrs.join(', ')}` : '';
    ...
    regexToString(body) {
        ...
        return `re.compile(r"${(0, string_utils_1.normalizeEscapedRegexQuotes)(body.source).replace(/\\\//, '/').replace(/"/g, '\\"')}"${suffix})`;
    }
    toCallWithExact(method, body, exact) {
        if (isRegExp(body))
...
class JavaLocatorFactory {
    generateLocator(base, kind, body, options = {}) {
        ...
        switch (kind) {
            ...
            case 'role':
                ...
                for (const { name, value } of options.attrs)
                    attrs.push(`.set${(0, string_utils_1.toTitleCase)(name)}(${typeof value === 'string' ? this.quote(value) : value})`);
                const attrString = attrs.length
                    ? `, new ${clazz}.GetByRoleOptions()${attrs.join('')}`
                    : '';
                return `getByRole(AriaRole.${(0, string_utils_1.toSnakeCase)(body).toUpperCase()}${attrString})`;
            case 'has-text':
    ...
    regexToString(body) {
        ...
        return `Pattern.compile(${this.quote((0, string_utils_1.normalizeEscapedRegexQuotes)(body.source))}${suffix})`;
    }
    toCallWithExact(clazz, method, body, exact) {
        if (isRegExp(body))
            return `${method}(${this.regexToString(body)})`;
        if (exact)
            return `${method}(${this.quote(body)}, new ${clazz}.${(0, string_utils_1.toTitleCase)(method)}Options().setExact(true))`;
        return `${method}(${this.quote(body)})`;
    }
    toHasText(body) {
        if (isRegExp(body))
...
class CSharpLocatorFactory {
    generateLocator(base, kind, body, options = {}) {
        switch (kind) {
            ...
            case 'role':
                ...
                for (const { name, value } of options.attrs)
                    attrs.push(`${(0, string_utils_1.toTitleCase)(name)} = ${typeof value === 'string' ? this.quote(value) : value}`);
                const attrString = attrs.length ? `, new() { ${attrs.join(', ')} }` : '';
                return `GetByRole(AriaRole.${(0, string_utils_1.toTitleCase)(body)}${attrString})`;
            case 'has-text':
    ...
    regexToString(body) {
        ...
        return `new Regex(${this.quote((0, string_utils_1.normalizeEscapedRegexQuotes)(body.source))}${suffix})`;
    }
    toCallWithExact(method, body, exact) {
        if (isRegExp(body))
            return `${method}(${this.regexToString(body)})`;
